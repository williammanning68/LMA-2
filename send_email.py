...
def _build_file_section_html(filename: str, matches):
    ...
    for idx, (_kw_set, excerpt_html, speaker, line_list, _s, _e) in enumerate(matches, 1):
        ...
        card = (
            ...
            "padding:9px 12px;mso-line-height-rule:exactly;vertical-align:middle;'>"
              "<table role='presentation' width='100%' cellpadding='0' cellspacing='0' border='0' "
              "style='border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;'>"
              "<tr valign='middle' height='24' style='height:24px;'>"
                "<td width='24' height='24' align='center' valign='middle' "
                "style='height:24px;background:#4A5A6A;border:0;vertical-align:middle;'>"
                  "<p style=\"margin:0;font:bold 10pt 'Segoe UI',sans-serif;color:#FFFFFF;"
                  "line-height:24px;mso-line-height-rule:exactly;\">"
                  f"{idx}</p>"
                "</td>"
                "<td width='8' height='24' style='height:24px;font-size:0;line-height:0;vertical-align:middle;'>&nbsp;</td>"
                "<td height='24' valign='middle' style='height:24px;vertical-align:middle;'>"
                  "<p style=\"margin:0;font:bold 10pt 'Segoe UI',sans-serif;color:#24313F;"
                  "text-transform:uppercase;line-height:24px;mso-line-height-rule:exactly;\">"
                  f"{esc(speaker) if speaker else 'UNKNOWN'}</p>"
                "</td>"
                "<td height='24' align='right' valign='middle' style='height:24px;vertical-align:middle;'>"
                  "<p style=\"margin:0;font:10pt 'Segoe UI',sans-serif;color:#6A7682;"
                  "line-height:24px;mso-line-height-rule:exactly;\">"
                  f"{line_txt}</p>"
                "</td>"
              "</tr>"
